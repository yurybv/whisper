...
The OpenAI key remains in Keychain and is added to requests only at execution time. Production sources contain no runtime logging calls, provider error bodies are discarded before errors reach UI or diagnostic formatting, and transformation requests use `store: false`. Runtime API traffic defaults only to `https://api.openai.com/v1`. App-owned audio directories under `Library/Application Support/Whisper` are restricted to the current user (`0700`); UUID-bound relative paths and cleanup tombstones keep reads and confirmed deletion inside the owned recording root. SwiftData metadata uses its platform-managed store separately from large audio files by design.
