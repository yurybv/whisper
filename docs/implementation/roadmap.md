...
Progress: `WH-M3-001` remains in review as of 2026-09-12. Live Microphone and Screen Recording grant/relaunch recovery was verified on 2026-09-07. Shortcut recovery exposed stale Input Monitoring consent and missing source retries; recovery corrections pass automated checks. On 2026-09-08 the owner reported that physical Command–Shift–K opened Finder Network; active command filtering, shortcut reassignment and key-lifecycle recovery corrected propagation. Native tracing on 2026-09-11 then proved the shortcut reached the mode-switcher presentation path and isolated the remaining zero-size placement defect. The switcher now selects and centers itself on the Mac's built-in display after final layout. The full scheme passes 170 unit tests and 4 UI tests. The exact rebuilt bundle has refreshed Accessibility and Input Monitoring grants, an enabled active tap, and direct presentation/Escape QA at the expected `560×452` built-in-display frame. Only a final physical Command–Shift–K check remains because available synthetic events bypass the session tap. `WH-M3-002` remains blocked.
