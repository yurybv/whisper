...
- **Blockers:** Final physical Command–Shift–K and live setup inspection are pending. Input Monitoring is granted to the corrected QA build; native diagnostics confirm an enabled tap with all three keyboard event types.
...
- **Remaining acceptance (2026-09-07):** The corrected QA bundle at `/tmp/whisper-recovery-tests/Build/Products/Debug/Whisper.app` has Input Monitoring enabled. Its running process has an enabled native tap with mask 7168 (key-down, key-up, flagsChanged), replacing the earlier disabled mask-4096 state. Computer-use key injection did not open the switcher; the owner has been asked to press Command–Shift–K physically and open the main window for final live inspection. Do not mark done or unblock WH-M3-002 until that check passes.
