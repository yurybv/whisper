...
## Control-Command-M implementation verification — 2026-09-12

- Replaced the Finder-conflicting Change Mode default with Control-Command-M across runtime settings, onboarding, menu-bar guidance, product source-of-truth documents, and regression tests. The legacy Command-Shift-K default no longer routes Change Mode.
- Replaced the ineffective `NSRunningApplication.current.activate(options:)` self-activation call with forced `NSApplication.activate(ignoringOtherApps: true)`. Live inspection established that the accessory app must first order a visible panel; activation is therefore requested after `orderFrontRegardless()`, followed by immediate and deferred key-window ordering. Prior-application focus restoration is retained.
- TDD evidence: shortcut tests first failed because key M was unavailable; onboarding UI first failed because it still displayed Command-Shift-K; the application activator seam first failed to compile; forced activation then failed against the cooperative API; and the final ordering assertion observed the panel hidden at activation before the production order was corrected. The completed focused shortcut groups passed **40 tests**, overlay lifecycle passed **10 tests**, onboarding passed **1 UI test**, and mode-switcher regressions passed **2 UI tests**, all with zero failures.
- The final complete scheme passed **171 unit tests and 4 UI tests** (**175 total**), zero failures. Result bundle: `/Users/yurybogdanov/Library/Developer/Xcode/DerivedData/Whisper-cvohsozztgjbkycfgedqypyhclkz/Logs/Test/Test-Whisper-2026.09.12_17-37-42-+0400.xcresult`.
- Built the exact QA app at `/tmp/whisper-control-command-m/Build/Products/Debug/Whisper.app`; the build succeeded and `codesign --verify --deep --strict` passed. Its identifier is `dev.yury.whisper` and its signature is ad hoc.
- With the owner's explicit approval, reset only `dev.yury.whisper` Input Monitoring and Accessibility records, added the exact QA bundle to both lists, and verified both switches on. No unrelated permission entry changed. The normal process then owned one enabled event tap with keyboard mask `7168` and options `0`.
- Production smoke testing reproduced the invisible panel and isolated two activation requirements: cooperative `NSApplication.activate()` was insufficient, and forced activation was ignored when requested before the accessory app owned a visible window. After the final ordering correction, Whisper became frontmost and exposed one onscreen layer-3 window at `(584, 349, 560, 452)` on the built-in display.
- The owner physically pressed Control-Command-M with Finder visible. The submitted screenshot shows the centered switcher, Search modes field, active Default mode, and keyboard guidance; Finder did not handle the shortcut and Rectangle did not react. The existing mode-switcher UI regressions verify immediate keyboard delivery to Search modes. The owner then pressed Escape; read-only inspection found zero onscreen Whisper windows, an external application frontmost, and the event tap still enabled.

Task is **done** and `WH-M3-002` is unblocked. No shortcut events, search text, dictated content, transcript, instruction, key, or Authorization header were recorded.
