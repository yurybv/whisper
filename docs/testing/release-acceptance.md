...
Overall result: **BLOCKED — current packaged-app foreground session not run**

Date: 2026-09-15

Environment: Apple Silicon MacBook Pro; macOS 26.4.1; Xcode 26.6 (17F113); Swift 6.3.3; macOS SDK 26.5; XcodeGen 2.46.0.

Build under test: commit `1fda779`; ad-hoc `build/Whisper.app`; bundle identifier `dev.yury.whisper`; arm64.
...
| ID | Case | Result | Evidence and remaining live check |
|---|---|---|---|
| X-01 | Clean supported-Mac build | PASS | `scripts/verify.sh --skip-ui-tests` passed twice from removed generated state; each run rebuilt and signed `build/Whisper.app`. |
...
| X-04 | Move exact bundle to `/Applications` | NOT RUN | Requires an installation session and may replace an existing user app. |
| X-05 | First launch through right-click Open | NOT RUN | Requires owner-visible Gatekeeper confirmation and foreground interaction. |
...
| X-07 | API key survives relaunch in Keychain and never appears in logs | AUTOMATED PASS / LIVE NOT RUN | Keychain round-trip and safe-error tests pass; the runtime has no logging APIs; perform a sanitized relaunch/log inspection without recording the key. |

## Blocking release session

Failed criterion: `WH-M6-003` requires every manual row to pass or fail on the current packaged version. The rows marked `NOT RUN` or `LIVE NOT RUN` cannot be promoted using automated evidence alone.

Reason: the owner asked development not to control the cursor, launch interactive UI, or interrupt foreground work. The remaining checks necessarily activate Whisper and target applications; permission and Gatekeeper rows also require owner confirmation.

Affected tasks: `WH-M6-003`, `WH-M6-005`, and `WH-M6-006`.

Recommended default: reserve one foreground acceptance session using only generated text/audio, run D-01 through X-07 on the then-current packaged commit, and record only outcomes and sanitized notes here.

Required external change: the owner explicitly indicates that a foreground QA session is available and handles Gatekeeper and macOS permission confirmations. No credential needs to be disclosed or recorded.
